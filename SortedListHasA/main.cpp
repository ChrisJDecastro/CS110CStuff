...
{
	cout << "The high score list contains " << endl;
...
void existingScoreList(SortedListInterface<int>* listPtr)
{
  int scores[] = {42, 30, 1, 1337};
  for (int i = 0;i<4;i++)
  {
    listPtr->insertSorted(scores[i]);
  }
...
int main()
{
   ...
   int score = 0;
   SortedListInterface<int>* listPtr = new SortedListHasA<int>();

   existingScoreList(listPtr);
   displayList(listPtr);
   while (score >= 0)
   {
      cout << "Please enter your score(DON'T LIE)" << endl;
      cout << "If you enter a negative score, you broke the game and the program will end." << endl;
      cin >> score;
      if(score <= 1000000000 && score >= 0)
      {
         listPtr->insertSorted(score);
         displayList(listPtr);
      }
   } 
